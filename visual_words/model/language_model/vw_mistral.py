...
from ..vw_arch import vwMetaModel, vwMetaForCausalLM

class LlavaConfig(MistralConfig):
    model_type = "llava"

class vwMistralModel(vwMetaModel, MistralModel):
    config_class = LlavaConfig

    def __init__(self, config: MistralConfig):
        super(vwMistralModel, self).__init__(config)


class vwMistralForCausalLM(MistralForCausalLM, vwMetaForCausalLM):
    config_class = LlavaConfig

    def __init__(self, config):
        super(MistralForCausalLM, self).__init__(config)
        self.model = vwMistralModel(config)
    ...
    def forward(
        ...
        input_ids, attention_mask, past_key_values, inputs_embeds, labels, image_position_labels = self.prepare_inputs_labels_for_multimodal(input_ids, attention_mask, past_key_values, labels, images)
        ###
        if image_position_labels is not None:
            image_position_labels=image_position_labels.bool()
            visual_labels=self.vm_head(inputs_embeds)
        else:
            visual_labels=None
        ###
